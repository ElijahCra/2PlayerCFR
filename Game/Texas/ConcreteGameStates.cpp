...
void ActionStateNoBet::enter(Game *game, Game::Action action) {
    ...
    if (0 == game->currentRound) {
        if (Game::Action::None == action) {
            game->setActions({Game::Action::Raise, Game::Action::Fold, Game::Action::Call});
...
void ActionStateNoBet::transition(Game *game, Game::Action action) {
    switch (action) {
        case Game::Action::Call: { //first action small-blind calls/limps
            game->setState(ActionStateNoBet::getInstance(), action);
            break;
        }
        case Game::Action::Check: {
            if (0 == game->currentRound){ //preflop 2nd player checks
                game->setState(ChanceState::getInstance(), action);
            } else if (1 == game->currentPlayer){ //postflop 1st action
                game->setState(ActionStateNoBet::getInstance(), action);
            } else if (0 == game->currentPlayer) { //postflop 2nd check
        ...
        case Game::Action::Fold: {//first action small-blind folds
            game->setState(TerminalState::getInstance(), action);//
            break;
        }
        case Game::Action::Raise: {//raise at any point
...
void ActionStateBet::enter(Game *game, Game::Action action) {
    if (Game::Action::Raise == action) {
        game->setActions(std::vector<Game::Action>{Game::Action::Reraise, Game::Action::Fold, Game::Action::Call});
