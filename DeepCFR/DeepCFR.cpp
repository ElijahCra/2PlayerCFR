...
template<typename GameType>
void DeepRegretMinimizer<GameType>::Train(uint32_t iterations, bool coro) {
        ...
        for (int p = 0; p < GameType::PlayerNum; ++p) {
            ...
            for (int k = 0; k < K_TRAVERSALS; ++k) {
                ...
                if (coro == false) {
                    ...
                    traverse_cfr_coro(m_game, p, iter, 1.0f);
                }
            }
            m_game.reInitialize();
...
template<typename GameType>
float DeepRegretMinimizer<GameType>::traverse_cfr_coro(const GameType& game, int updatePlayer, int current_iter, float probUpdatePlayer) {
    // Terminal node
    if (game.getType() == "terminal") {
        return game.getUtility(updatePlayer);
    }
